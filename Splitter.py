...
# Attempt to create EasyOCR reader (RU is used in your original code)
try:
    ...
    try:
        ...
        ocr_engine = None  # graceful fallback if easyocr not available
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        ...
        cfg = {}
    # backward compatibility: convert old "jobs" to "jobs_conditions" if needed
...
def ocr_header(image, header_ratio=0.15):
    """Return text from top header crop (image is OpenCV BGR array)."""
    h = image.shape[0]
...
def correct_orientation(image):
    """Placeholder for any orientation correction. Currently no-op."""
    return image


def extract_text_with_ocr(page):
    """Extract text from a fitz page using EasyOCR (fallbacks handled)."""
...
def check_include(text, inc_list, all_must_match):
    """Check include conditions (strings or lists of alternatives)."""
    if not inc_list:
        return False
    matches = []
...
# ---------------------------- PDF splitting with empty-page handling ----------------------------
...
def split_pdfs(pdf_paths, conds_dict, output_dir, progress_callback=None):
    """
    Splits given PDFs according to conds_dict into output_dir.
    Additionally, detects pages with no text (even after OCR) and saves them to:
        <output_dir>/Tühjad_lehed/<original_basename>_page_<N>.pdf
    Returns a mapping of results and also includes empty pages list under special key '___EMPTY_PAGES___'.
    """
    all_results = {}
    empty_root = os.path.join(output_dir, "Tühjad_lehed")
    os.makedirs(empty_root, exist_ok=True)

    for pdf_path in pdf_paths:
        results = {}
        empty_pages = []  # list of saved empty page paths for this pdf
        try:
            doc = fitz.open(pdf_path)
        except Exception as e:
            all_results[os.path.basename(pdf_path)] = {"__error__": str(e)}
            continue
        total_pages = doc.page_count

        # find fallback subjob (one with no include/exclude) if any
        fallback_sj = None
        ...
        def save_batch(batch_sj, batch_pages, keywords):
            if not batch_pages:
                return
            safe_sj = safe_name(batch_sj or "unknown_job")
            ...
            if len(batch_pages) == 1:
                out_name = f"{safe_base_name}_page_{batch_pages[0]+1}.pdf"
            else:
                out_name = f"{safe_base_name}_pages_{batch_pages[0]+1}_{batch_pages[-1]+1}.pdf"
            out_path = os.path.join(sj_dir, out_name)

            # avoid overwrite
            counter = 1
            base_out, ext = os.path.splitext(out_path)
            while os.path.exists(out_path):
                out_path = f"{base_out}_{counter}{ext}"
                counter += 1
        ...
        for i in range(total_pages):
            ...
            # if still empty -> mark as empty and save separately
            if not text.strip():
                # save single-page PDF to empty_root
                base_name = os.path.splitext(os.path.basename(pdf_path))[0]
                safe_base = safe_name(base_name)
                empty_name = f"{safe_base}_page_{i+1}.pdf"
                empty_path = os.path.join(empty_root, empty_name)
                # avoid overwrite
                c = 1
                be, ex = os.path.splitext(empty_path)
                while os.path.exists(empty_path):
                    empty_path = f"{be}_{c}{ex}"
                    c += 1
                writer = fitz.open()
                writer.insert_pdf(doc, from_page=i, to_page=i)
                writer.save(empty_path)
                writer.close()
                empty_pages.append(empty_path)
                # do not include this page in normal batches
            ...
            if matched_sj:
                # if rule changed, flush previous batch
                if current_batch:
        ...
        # attach empty pages info into results under a special key
        if empty_pages:
            results.setdefault("___EMPTY_PAGES___", []).extend(empty_pages)
...
class ModernPDFSplitter:
    ...
    def setup_ui(self):
        ...
        self.label_output = tk.Label(top_frame, text="Väljundkaust: [not set]", bg="#f7f9fc")
    ...
    def refresh_tree(self):
        """TreeView uuendamine – subjobid tähestikulises järjekorras ja vaikimisi suletud"""
        self.tree.delete(*self.tree.get_children())
        mains = list(self.jobs.keys())
        # sort main jobs case-insensitively so UI is more stable (optional)
        for mainjob in mains:
            parent = self.tree.insert("", "end", text=mainjob, open=False)  # closed by default
            subs = self.jobs.get(mainjob, {})
            for subjob in sorted(subs.keys(), key=lambda x: x.casefold()):
    ...
    def _finish_split(self, mainjob, results, out_dir):
        ...
        for pdfname, data in results.items():
            for sj, files in data.items():
                if sj == "___EMPTY_PAGES___":
                    continue
                parts.append(f"{pdfname} - {sj}: {len(files)} faili")
            # collect empty pages
            ep = data.get("___EMPTY_PAGES___") or data.get("___EMPTY_PAGES__", []) or []
            if ep:
                empty_pages_count += len(ep)
                empty_pages_paths.extend(ep)

        # move original files to processed_dir (if set)
        if self.processed_dir and self.pdf_list:
            for pdf in self.pdf_list:
                try:
                    dest = os.path.join(self.processed_dir, os.path.basename(pdf))
                    # avoid overwrite
                    c = 1
        ...
        if parts:
            ...
            if empty_pages_count:
                extra = f"\n\nTuvastatud tühjad lehed: {empty_pages_count}\nAsuvad kaustas: {os.path.join(out_dir, 'Tühjad_lehed')}"
            messagebox.showinfo("Done", f"Main Job '{mainjob}' splititud.\n\n{msg}{extra}\n\nVäljund: {out_dir}")
        else:
            # no normal parts; still report empty pages if any
            if empty_pages_count:
                messagebox.showinfo("Tulemused", f"Pole loodud tavapäraseid split-failisid, kuid leiti {empty_pages_count} tühja lehte.\nAsuvad: {os.path.join(out_dir, 'Tühjad_lehed')}")
            else:
                messagebox.showwarning("Tulemus", "Ühtegi faili ei loodud (pole matching rule'i või lehti).")
